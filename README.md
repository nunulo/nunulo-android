...
- Nunulo 稳定服务已上线，当前待发布版本为 `v0.2.5`；历史 APK 不代表当前版本。
...
- 当前版本身份为 `0.2.5`（`versionCode=10`）；首次安装或会话失效时直接进入登录页，支持公开注册或邀请码注册，物理设备上的原生地图、拍照和断网恢复仍需持续回归。
...
Android 现有五个入口：动态、地图、记录、消息、我的。动态支持全部、关注、公开、我的四种范围；地图支持我的、关注、发现三种范围，并继续使用服务端可见性与位置降精度结果；记录和编辑支持仅自己、关注者、所有成员三档可见性，其中只有用户逐条开启“互联网匿名展示”的公开记录会进入匿名窗口。详情支持点赞、评论、举报、本人编辑删除和加入合集。消息显示未读数量和成员，个人页提供存储额度、合集、数据导出下载分享和个人邀请码。

进入地图页会自动请求定位并显示当前位置；进入记录页且草稿坐标为空时会自动填入设备坐标，定位按钮保留为重新定位入口。空记录地图不再移动到任何写死城市。x86_64 模拟器不支持当前高德原生 SDK 时只显示真实坐标列表，不绘制伪造底图或随机点；高德原生地图仍必须在受支持的 ARM Android 设备验收。
