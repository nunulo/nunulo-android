...
- 当前中期展示版身份为 `0.3.0-preview.1`（`versionCode=12`）；首次安装或会话失效时直接进入登录页，支持公开注册或邀请码注册。Linux 自动测试、构建与界面回归是本轮门禁；物理设备上的原生地图、拍照和断网恢复保留为后续 L4 证据。正式 `0.3.0` 必须使用更高的 `versionCode`。
