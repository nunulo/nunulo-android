...
“我的 → 社区”提供可浏览的成员目录：可按显示名、用户名和简介搜索，筛选已关注成员，并从成员卡打开完整主页。成员主页显示头像、简介、真实关注统计和当前账号有权查看的照片记录；关注、取消关注和屏蔽继续服从服务端权限与幂等边界。即使旧生产 API 忽略作者筛选参数，客户端仍会按返回记录的真实作者再次过滤，不会把其他成员内容混入个人主页。
