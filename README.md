...
- Nunulo 邀请制服务已上线，当前稳定版本为 `v0.2.4`；历史 APK 不代表当前版本。
...
- 当前版本身份为 `0.2.4`（`versionCode=9`）；首次安装或会话失效时直接进入登录页，物理设备上的原生地图、拍照和断网恢复仍需持续回归。
...
Android 现有五个入口：动态、地图、记录、消息、我的。动态支持全部、关注、公开、我的四种范围；地图支持我的、关注、发现三种范围，并继续使用服务端可见性与位置降精度结果；记录和编辑支持仅自己、关注者、所有成员三档可见性。详情支持点赞、评论、举报、本人编辑删除和加入合集。消息显示未读数量和成员，个人页提供存储额度、合集、数据导出下载分享和个人邀请码。
